...
case "${INCLUDE_OPTIONS}" in
    1|true|TRUE|yes|YES|on|ON)
        INCLUDE_OPTIONS_ARGS=(--include-options)
...
echo "$(date '+%Y-%m-%dT%H:%M:%S%z') [INGEST] config max_secs_per_ticker=${INGEST_MAX_SECS_PER_TICKER} price_timeout_sec=${PRICE_TIMEOUT_SEC} price_retry_attempts=${PRICE_RETRY_ATTEMPTS} retry_failed_passes=${INGEST_RETRY_FAILED_PASSES} retry_failed_backoff_sec=${INGEST_RETRY_FAILED_BACKOFF_SEC} require_full_dataset=${REQUIRE_FULL_DATASET} include_options=${INCLUDE_OPTIONS_BOOL} enabled=${RUN_INGEST}" >> "$RUN_LOG"
